    ...
    while queue:
        ...
        for idx in range(4):
            ...
            if 0 <= ii < N and 0 <= jj < N and visited[ii][jj] == 0:
				# 5. 간선은 상하 좌우지만 조건에 따라서 움직이기 때문에 조건을 상세하여야한다. -> 1. TODO BFS 안에서 로직 세분화 (상어 크기를 기준으로 비교)
    
                    # 1) 상어 크기 보다 작고, 0이 아니면 후보 리스트에 넣기 (단, 큐엔 넣지 않음)
                if space[x][y] > space[ii][jj] and space[ii][jj] != 0:
                    visited[ii][jj] =  visited[i][j] + 1
                    cand.append((visited[ii][jj] - 1, ii, jj))
                    # 2) 같으면 지나칠 수 는 있기 때문에
                elif space[x][y] == space[ii][jj]:
                    visited[ii][jj] =  visited[i][j] + 1
                    queue.append([ii,jj])
                    # 3) 0인 경우는 걍 다넣음 -> 어짜피 방문처리때메 또 안들어옴
                elif space[ii][jj] == 0:
                    visited[ii][jj] =  visited[i][j] + 1
                    queue.append([ii,jj])
                    
	# 6. 후보 리스트는 우선 순위가 있기 때문에 정렬을 사용할 수 있다. -> 2. TODO 이걸 놓침
